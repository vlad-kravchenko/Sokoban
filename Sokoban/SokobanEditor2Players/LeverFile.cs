...
namespace SokobanEditor2Players
{
    public class LeverFile
    {
        ...
        public Cell [,] LoadLevel(int level)
        {
            ...
            try
            {
                if (!File.Exists(filename)) File.WriteAllLines(filename, null);
            ...
            if (cells == null)
            {
                ...
                lines[currentLine] = curLevel + 1 + " " + newLevelSize + " " + newLevelSize;
                for (int i = 0; i < newLevelSize; i++)
                {
                    lines[currentLine + i + 1] = new string(' ', newLevelSize);
                }
