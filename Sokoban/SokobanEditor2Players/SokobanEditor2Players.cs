...
{
    public partial class SokobanEditor2Players : Form
    {
        #region Attributes
        ...
        private void SokobanEditor2Players_Resize(object sender, EventArgs e)
        {
            int bw, bh;
        ...
        private void SokobanEditor2Players_Load(object sender, EventArgs e)
        {
        ...
        private void SetCurrentCell(Cell selectedCell)
        {
            ...
            toolUser.Checked = currentCell == Cell.User1;
        ...
        {
            if (cell == Cell.User1) RemoveItem(Cell.User1);
            if (cell == Cell.User2) RemoveItem(Cell.User2);
        ...
        private string IsGoodLevel()
        {
            ...
            if (users == 0) return "Нужно указать начальное место первого игрока!";
            if (users > 1) return "Нужно указать только одного первого игрока!";

            users = CountItems(Cell.User2);
            if (users == 0) return "Нужно указать начальное место второго игрока!";
            if (users > 1) return "Нужно указать только одного второго игрока!";
